<h5 class="font-20 mt-15 mb-1"><?php echo str_replace('_',' ','Send'); ?></h5>
<?php
echo $this->session->flashdata('msg');
?>
<!--Action-->
<div>
	<div class="float_left padding_10">
		<a href="<?php echo site_url('admin/send/save'); ?>"
			class="btn btn-success">Add</a>
	</div>
	<div class="float_left padding_10">
		<i class="fa fa-download"></i> Export <select name="xeport_type"
			class="select"
			onChange="window.location='<?php echo site_url('admin/send/export'); ?>/'+this.value">
			<option>Select..</option>
			<option>Pdf</option>
			<option>CSV</option>
		</select>
	</div>
	<div class="float_right padding_10">
		<ul class="left-side-navbar d-flex align-items-center">
			<li class="hide-phone app-search mr-15">
                <?php echo form_open_multipart('admin/send/search/',array("class"=>"form-horizontal")); ?>
                    <input name="key" type="text"
				value="<?php echo isset($key)?$key:'';?>" placeholder="Search..."
				class="form-control">
				<button type="submit" class="mr-0">
					<i class="fa fa-search"></i>
				</button>
                <?php echo form_close(); ?>
            </li>
		</ul>
	</div>
</div>
<!--End of Action//-->

<!--Data display of send-->
<table class="table table-striped table-bordered">
	<tr>
		<th>Agent Users</th>
		<th>Subject</th>
		<th>Note</th>
		<th>Phone No</th>
		<th>Amount</th>
		<th>Assign</th>
		<th>Status</th>

		<th>Actions</th>
	</tr>
	<?php foreach($send as $c){ ?>
    <tr>
		<td><?php
    $this->CI = & get_instance();
    $this->CI->load->database();
    $this->CI->load->model('Users_model');
    $dataArr = $this->CI->Users_model->get_users($c['agent_users_id']);
    echo $dataArr['email'];
    ?>
									</td>
		<td><?php echo $c['subject']; ?></td>
		<td><?php echo $c['note']; ?></td>
		<td><?php echo $c['phone_no']; ?></td>
		<td><?php echo $c['amount']; ?></td>
		<td>
		 <?php echo form_open_multipart('admin/send/assign/'.$c['id'],array("class"=>"form-horizontal")); ?>
		<div class="form-group">
			<label for="Assigned Users" class="col-md-4 control-label">Assigned
				Agent</label>
			<div class="col-md-8"> 
          <?php
        $this->CI = & get_instance();
        $this->CI->load->database();
        $this->CI->load->model('Users_model');
        $dataArr = $this->CI->Users_model->get_all_users();
        ?> 
          <select name="agent_users_id" id="agent_users_id"
					class="form-control" />
				<option value="">--Select--</option> 
            <?php
            for ($i = 0; $i < count($dataArr); $i ++) {
                ?> 
            <option value="<?=$dataArr[$i]['id']?>"
					<?php if($c['assigned_users_id']==$dataArr[$i]['id']){ echo "selected";} ?>><?=$dataArr[$i]['first_name']?> <?=$dataArr[$i]['last_name']?></option> 
            <?php
            }
            ?> 
          </select>
          <input type="hidden" name="phone_no" value="<?php echo $c['phone_no']; ?>">
          <input type="hidden" name="amount" value="<?php echo $c['amount']; ?>">
          <input type="submit" value="Submit">
			</div>
		</div>
		<?php echo form_close(); ?>
		
		
		<?php
    $this->CI = & get_instance();
    $this->CI->load->database();
    $this->CI->load->model('Users_model');
    $dataArr = $this->CI->Users_model->get_users($c['assigned_users_id']);
    echo $dataArr['first_name'].' '.$dataArr['last_name'];
    ?>
									</td>
		<td><?php echo $c['status']; ?></td>

		<td><a href="<?php echo site_url('admin/send/details/'.$c['id']); ?>"
			class="action-icon"> <i class="zmdi zmdi-eye"></i></a> <a
			href="<?php echo site_url('admin/send/save/'.$c['id']); ?>"
			class="action-icon"> <i class="zmdi zmdi-edit"></i></a> <a
			href="<?php echo site_url('admin/send/remove/'.$c['id']); ?>"
			onClick="return confirm('Are you sure to delete this item?');"
			class="action-icon"> <i class="zmdi zmdi-delete"></i></a></td>
	</tr>
	<?php } ?>
</table>
<!--End of Data display of send//-->

<!--No data-->
<?php
if (count($send) == 0) {
    ?>
<div align="center">
	<h3>Data is not exists</h3>
</div>
<?php
}
?>
<!--End of No data//-->

<!--Pagination-->
<?php
echo $link;
?>
<!--End of Pagination//-->
